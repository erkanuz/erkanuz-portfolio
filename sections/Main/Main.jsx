'use client';
import React, { useRef } from "react";
import styles from "./style.module.scss";

import Link from 'next/link'
import { AnimatedText, MagneticIcon } from '@/components';

import gsap from "gsap";
import { useGSAP } from "@gsap/react";

const tags = [
	"Data Analysis",
	"Business Intelligence",
	"Banking",
	"Web Development",
	"Design",
];

const SocialLink = ({ href, icon }) => (
	<Link href={href}>
		<MagneticIcon>
			<i className="socialLink">{icon}</i>
		</MagneticIcon>
	</Link>
);

export const Main = () => {

	const container = useRef();
	const tl = useRef();

	useGSAP(() => {
		gsap.set(".Title", { xPercent: -150, opacity: 0 });
		gsap.set(".mainDescription", { xPercent: -150, opacity: 0 });
		gsap.set(".kicker", {skewX: 0, opacity: 0 });
		gsap.set(".AnimatedImage", { yPercent: -150, opacity: 0 });
		gsap.set(".Tags", { xPercent: 20, opacity: 0 });
		gsap.set([".button", ".contact"], { scaleX: 0, opacity: 0 });
		gsap.set(".socialLink", { xPercent: 20, opacity: 0 });

		tl.current = gsap
			.timeline({ defaults: { duration: 1, ease: 'power4.inOut' } })
			.to('.Title', { xPercent: 0, opacity: 1, duration: 2, stagger: { each: .9, grid: 'auto', from: 'edges' }  })
			.to(".mainDescription", { xPercent: 0, opacity: 1 })
			.to('.kicker', { skewX: 1, opacity: 1 })
			.to('.AnimatedImage', { yPercent: 0, opacity: 1 })
			.to('.Tags', { xPercent: 0, opacity: 1, stagger: { each: 0.09, grid: 'auto', from: 'random' } })
			.to(['.button', '.contact'], { scaleX: 1, opacity: 1, stagger: { each: 0.09, grid: 'auto', from: 'edges' } }) 
			.to('.socialLink', { xPercent: 0, opacity: 1, stagger: { each: 0.09, grid: 'auto', from: 'random' } })
	}, { scope: container });


	return (
		<main className={styles.page}>
			<section className={styles.hero} ref={container}>
				<div className={styles.copy}>
					<div className={`${styles.title} Title`}>
						<span data-target="target" className={styles.name}>Erkan</span>

						<div className={styles.surname}>
							<span data-target="target">Uzunhalil</span>

							<img
								src="/logo.png"
								alt="Erkan Uzunhalil logo"
								className={styles.logo_img}
							/>
						</div>
					</div>

					<h1 className={`${styles.kicker} kicker`}>
						Technology, Data &amp; Finance
					</h1>

					<div className={`${styles.description} mainDescription`}>
						<AnimatedText />
					</div>

					<div className={styles.tags}>
						{tags.map((tag) => (
							<span key={tag} className={`${styles.tag} Tags`}>
								{tag}
							</span>
						))}
					</div>

					<div className={styles.actions}>
						<Link href="/projects" className={`${styles.button} button`}>
							VIEW PROJECTS
							<span className={styles.arrow}>→</span>
						</Link>

						<a href="#contact" className={`${styles.contact} contact`}>
							CONTACT ME
						</a>
					</div>
				</div>

				<div className={styles.illustration}>
					<img
						src="/images/Animated.svg"
						alt="Data analyst working with financial dashboards"
						width={511}
						height={454}
						className="AnimatedImage"
					/>

					<div className={styles.mini_box}>
						<div className={styles.socials}>
							<SocialLink
								href="https://github.com/erkanuz"
								icon={
									<>
										<svg
											width="30"
											height="30"
											viewBox="0 0 24 24"
											fill="none"
											xmlns="http://www.w3.org/2000/svg"
										>
											<path
												d="M12 0.296875C5.371 0.296875 0 5.66775 0 12.2979C0 17.6634 3.438 22.0916 8.207 23.6345C8.805 23.7445 9.025 23.3905 9.025 23.086C9.025 22.8151 9.016 22.0655 9.011 21.0713C5.957 21.7535 5.211 19.4892 5.211 19.4892C4.688 18.1378 3.901 17.8025 3.901 17.8025C2.821 17.1273 4.001 17.1428 4.001 17.1428C5.209 17.2244 5.847 18.3731 5.847 18.3731C6.929 20.2632 8.756 19.7216 9.404 19.4532C9.51 18.6797 9.819 18.1478 10.159 17.8725C7.386 17.6022 4.482 16.519 4.482 11.7709C4.482 10.4263 4.948 9.3295 5.729 8.4577C5.598 8.15274 5.188 6.89754 5.834 5.23768C5.834 5.23768 6.86 4.92936 8.989 6.57671C9.941 6.27633 10.973 6.13501 12 6.12996C13.027 6.13501 14.059 6.27633 15.011 6.57671C17.14 4.92936 18.166 5.23768 18.166 5.23768C18.812 6.89754 18.402 8.15274 18.271 8.4577C19.052 9.3295 19.518 10.4263 19.518 11.7709C19.518 16.5319 16.609 17.5953 13.83 17.8607C14.261 18.2187 14.636 18.9207 14.636 20.0004C14.636 21.4757 14.626 22.7252 14.626 23.0905C14.626 23.395 14.846 23.754 15.449 23.6376C20.213 22.0905 23.655 17.6613 23.655 12.2979C23.655 5.66775 18.285 0.296875 12 0.296875Z"
												fill="black"
											/>
										</svg>
										<span className={styles.hoverText}>GitHub</span>
									</>
								}
							/>
							<SocialLink
								href="https://leetcode.com/_erkan/"
								icon={
									<>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											x="0px"
											y="0px"
											width="30"
											height="30"
											viewBox="0 0 16 16"
										>
											<path d="M 10.447266 0.265625 A 0.50005 0.50005 0 0 0 10.103516 0.41796875 L 5.65625 4.8671875 L 3.0957031 7.4257812 A 0.50005 0.50005 0 0 0 2.9785156 7.6035156 C 1.769869 8.9739016 1.7865696 11.063913 3.0957031 12.373047 L 5.65625 14.933594 C 7.0176322 16.294976 9.242133 16.294976 10.603516 14.933594 L 12.853516 12.683594 A 0.50063809 0.50063809 0 1 0 12.144531 11.976562 L 9.8945312 14.226562 C 8.9159134 15.20518 7.3418991 15.20518 6.3632812 14.226562 L 3.8027344 11.666016 C 2.8241166 10.687398 2.8241166 9.1114303 3.8027344 8.1328125 L 6.3632812 5.5742188 C 7.3418991 4.5956009 8.9159135 4.5956009 9.8945312 5.5742188 L 12.144531 7.8242188 A 0.50063784 0.50063784 0 1 0 12.853516 7.1171875 L 10.603516 4.8671875 C 9.9106907 4.174363 8.9943718 3.8431189 8.0820312 3.8554688 L 10.8125 1.125 A 0.50005 0.50005 0 0 0 10.447266 0.265625 z M 7.328125 9.4003906 A 0.50005 0.50005 0 1 0 7.328125 10.400391 L 14.228516 10.400391 A 0.50005 0.50005 0 1 0 14.228516 9.4003906 L 7.328125 9.4003906 z"
												fill="black"></path>
										</svg>
										<span className={styles.hoverText}>Leetcode</span>
									</>
								}
							/>
							<SocialLink
								href="https://www.codewars.com/users/%20Qixing"
								icon={
									<>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											x="0px"
											y="0px"
											width="35"
											height="35"
											viewBox="0 0 24 24"
										>
											<path d="M 7 3 C 4.79 3 3 4.79 3 7 L 3 17 C 3 19.21 4.79 21 7 21 L 17 21 C 19.21 21 21 19.21 21 17 L 21 7 C 21 4.79 19.21 3 17 3 L 7 3 z M 11.564453 6.015625 C 11.709654 6.0045879 11.857203 6.004625 12.001953 6.015625 C 12.910953 6.061625 13.743656 6.5551406 14.222656 7.2441406 C 14.255656 7.2441406 14.288266 7.2382812 14.322266 7.2382812 C 15.522266 7.2272812 16.681594 7.9608125 17.183594 9.0078125 C 17.597594 9.8178125 17.587469 10.786875 17.230469 11.546875 C 17.247469 11.575875 17.269156 11.600859 17.285156 11.630859 C 17.894156 12.664859 17.838594 14.034188 17.183594 14.992188 C 16.688594 15.756187 15.844813 16.231734 15.007812 16.302734 C 14.991812 16.331734 14.979891 16.362625 14.962891 16.390625 C 14.371891 17.435625 13.156047 18.072375 11.998047 17.984375 C 11.089047 17.938375 10.256344 17.444859 9.7773438 16.755859 C 9.7443438 16.755859 9.7117344 16.761719 9.6777344 16.761719 C 8.4777344 16.772719 7.3184063 16.039187 6.8164062 14.992188 C 6.4024063 14.182187 6.4125313 13.213125 6.7695312 12.453125 C 6.7525312 12.424125 6.7308437 12.399141 6.7148438 12.369141 C 6.1058438 11.335141 6.1604062 9.9658125 6.8164062 9.0078125 C 7.3114063 8.2438125 8.1551875 7.7682656 8.9921875 7.6972656 C 9.0081875 7.6682656 9.0201094 7.638375 9.0371094 7.609375 C 9.5542344 6.695 10.548045 6.0928848 11.564453 6.015625 z M 12 7.2675781 C 11.285 7.3515781 10.652172 7.7564687 10.326172 8.3554688 C 9.9941719 8.9394687 9.9520938 9.6870313 10.246094 10.332031 C 10.449094 10.789031 10.820547 11.181984 11.310547 11.458984 C 11.320547 11.446984 11.331797 11.435828 11.341797 11.423828 C 11.029797 10.683828 11.032547 9.7621406 11.435547 8.9941406 C 11.777547 8.2861406 12.416156 7.7351719 13.160156 7.4511719 C 12.779156 7.2921719 12.381 7.2265781 12 7.2675781 z M 8.640625 8.7226562 C 8.312625 8.9726562 8.0573438 9.2847656 7.9023438 9.6347656 C 7.6173437 10.296766 7.6498594 11.044953 8.0058594 11.626953 C 8.3458594 12.206953 8.9746875 12.615594 9.6796875 12.683594 C 10.176687 12.736594 10.700547 12.612125 11.185547 12.328125 C 11.179547 12.312125 11.168109 12.29825 11.162109 12.28125 C 10.368109 12.17925 9.5782344 11.716328 9.1152344 10.986328 C 8.6732344 10.336328 8.514625 9.5096563 8.640625 8.7226562 z M 14.320312 8.7285156 C 13.647312 8.7335156 12.979359 9.0714375 12.568359 9.6484375 C 12.275359 10.049437 12.123 10.560187 12.125 11.117188 C 12.142 11.119188 12.158781 11.130766 12.175781 11.134766 C 12.660781 10.497766 13.456312 10.043812 14.320312 10.007812 C 15.104313 9.9498125 15.900531 10.227469 16.519531 10.730469 C 16.466531 10.321469 16.324609 9.9437656 16.099609 9.6347656 C 15.668609 9.0577656 15.003312 8.7115156 14.320312 8.7285156 z M 13.943359 11.308594 C 13.563578 11.334422 13.178203 11.458875 12.814453 11.671875 C 12.821453 11.687875 12.831891 11.70175 12.837891 11.71875 C 13.631891 11.82075 14.421766 12.283672 14.884766 13.013672 C 15.326766 13.663672 15.485375 14.490344 15.359375 15.277344 C 15.687375 15.027344 15.942656 14.715234 16.097656 14.365234 C 16.382656 13.703234 16.350141 12.955047 15.994141 12.373047 C 15.654141 11.793047 15.025312 11.384406 14.320312 11.316406 C 14.196063 11.303156 14.069953 11.299984 13.943359 11.308594 z M 12.689453 12.541016 C 12.679453 12.553016 12.668203 12.564172 12.658203 12.576172 C 12.971203 13.316172 12.967453 14.237859 12.564453 15.005859 C 12.222453 15.713859 11.583844 16.264828 10.839844 16.548828 C 11.220844 16.707828 11.618047 16.773422 11.998047 16.732422 C 12.713047 16.648422 13.347828 16.243531 13.673828 15.644531 C 14.005828 15.060531 14.045953 14.312969 13.751953 13.667969 C 13.549953 13.210969 13.179453 12.818016 12.689453 12.541016 z M 11.824219 12.865234 C 11.339219 13.502234 10.543687 13.956188 9.6796875 13.992188 C 8.8956875 14.050187 8.0994687 13.772531 7.4804688 13.269531 C 7.5334687 13.678531 7.6753906 14.056234 7.9003906 14.365234 C 8.3313906 14.942234 8.9957344 15.288484 9.6777344 15.271484 C 10.351734 15.266484 11.020641 14.928562 11.431641 14.351562 C 11.724641 13.950563 11.877 13.439813 11.875 12.882812 C 11.858 12.880812 11.841219 12.869234 11.824219 12.865234 z"
												fill="black"></path>
										</svg>
										<span className={styles.hoverText}>Codewars</span>
									</>
								}
							/>
							<SocialLink
								href="https://www.linkedin.com/in/erkan-uz-lil/"
								icon={
									<>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											x="0px"
											y="0px"
											width="30"
											height="30"
											viewBox="0 0 50 50"
										>
											<path d="M41,4H9C6.24,4,4,6.24,4,9v32c0,2.76,2.24,5,5,5h32c2.76,0,5-2.24,5-5V9C46,6.24,43.76,4,41,4z M17,20v19h-6V20H17z M11,14.47c0-1.4,1.2-2.47,3-2.47s2.93,1.07,3,2.47c0,1.4-1.12,2.53-3,2.53C12.2,17,11,15.87,11,14.47z M39,39h-6c0,0,0-9.26,0-10 c0-2-1-4-3.5-4.04h-0.08C27,24.96,26,27.02,26,29c0,0.91,0,10,0,10h-6V20h6v2.56c0,0,1.93-2.56,5.81-2.56 c3.97,0,7.19,2.73,7.19,8.26V39z"
												fill="black"></path>
										</svg>
										<span className={styles.hoverText}>Linkedin</span>
									</>
								}
							/>
						</div>
					</div>
				</div>
			</section>
		</main>
	);
};